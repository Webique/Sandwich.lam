...
hamburger.addEventListener('click', (e) => {
    ...
    navLinks.classList.toggle('mobile'); // Switch to mobile layout
...
navLinksItems.forEach(link => {
    link.addEventListener('click', (e) => {
        ...
        if (window.innerWidth <= 768) {
            offset += 20; // Add 20px for phones and smaller devices
        }
        ...
        navLinks.classList.remove('mobile'); // Close mobile menu
        navLinks.classList.remove('active'); // Remove active state
    });
});
